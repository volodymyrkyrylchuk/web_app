...
def get_profiles_list(request):
    ...
    return render(request, 'profiles.html', context={'result': objects, 'search': search})
...
def add_profile(request):
    if request.method == 'POST':
        ...
        form = ProfileAddForm()
    return render(request, template_name='profile_add.html', context={'form': form})


def get_publication(request, id):
    publication = Publication.objects.get(id=id)
    comments = publication.comments.all()
    return render(request, 'publication.html', context={'date': publication.create_date,
